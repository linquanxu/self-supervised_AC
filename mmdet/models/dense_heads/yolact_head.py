...
class YOLACTProtonet(BaseModule):
    ...
    def __init__(self,
        ...
        self.fp16_enabled = False
        self.pre_fc = nn.Linear(19044, 1)  ###afer segmtation for predicted，mobilenetv2:4900, resnet50:19044
        nn.init.kaiming_normal(self.pre_fc.weight)
        self.pre_loss = nn.SmoothL1Loss()
        # self.file_pre = open('file_pre.txt', 'w')
        self.is_computed_ac = 0
    ...
    def forward(self, x, coeff_pred, bboxes, img_meta, sampling_results=None):
        ...
        for idx in range(num_imgs):
            ...
            mask_pred_feature = self.crop(mask_pred_feature, bboxes_for_cropping)
            mask_pred_feature = mask_pred_feature.permute(2, 0, 1).contiguous()
            mask_pred_feature_list.append(mask_pred_feature)
        return mask_pred_list,mask_pred_feature_list

    @force_fp32(apply_to=('mask_pred', ))
    def loss(self, mask_pred, gt_masks, gt_bboxes, img_meta, sampling_results,mask_pred_feature):
        """Compute loss of the head.
        ...
        for idx in range(num_imgs):
            ...
            cur_mask_pred_feature = mask_pred_feature[idx]
            cur_gt_masks = gt_masks[idx].float()
            ...
            if num_pos == 0:
                ...
                if self.is_computed_ac:
                    a_pre_loss = 0
                    for p_i in range(num_pos):
                        pre_x = cur_mask_pred_feature[p_i]
                        pre_x = pre_x.view(1, -1)
                        pre_fc_value = self.pre_fc(pre_x)
                        pre_gt = torch.tensor(float(cur_img_meta['pre_value']), dtype=torch.float).cuda()
                        a_pre_loss += self.pre_loss(pre_fc_value, pre_gt)
                    a_pre_loss /= num_pos
                    loss += 1.5*a_pre_loss
    ...
    def simple_test(self,
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            ...
            segm_feature = [[[] for _ in range(self.num_classes)]
                            for _ in range(num_imgs)]
        else:
            ...
            mask_preds,mask_pred_feature = self.forward(feats[0], det_coeffs, _bboxes, img_metas)
            ...
            for i in range(num_imgs):
                if det_bboxes[i].shape[0] == 0:
                    ...
                    """
                    cur_mask_feature = mask_pred_feature[i]
                    num_pos = cur_mask_feature.shape[0]
                    avg_pre = 0
                    for x in range(num_pos):
                        pre_fea = cur_mask_feature[x]
                        pre_fea = pre_fea.view(-1,4900)
                        out = self.pre_fc(pre_fea)
                        avg_pre += out
                    avg_pre /= num_pos
                    self.file_pre.write(img_metas[i]['pre_value']+','+str(avg_pre.cpu().item())+'\n')
                    """
                    segm_feature.append(mask_preds[i])


        return segm_results, segm_feature
